...
Info
    Defines class MygenGen with attribute(s) and method(s).
...
try:
    ...
    from mygen.pro.read_template import ReadTemplate
    from mygen.pro.write_template import WriteTemplate
...
class MygenGen(FileCheck, ProConfig, ProName):
    '''
        Defines class MygenGen with attribute(s) and method(s).
        Generates module file by template and parameters.
    ...
    def gen_pro(self, pro_name: str | None, verbose: bool = False) -> bool:
        ...
        status: bool = False
        if bool(self.config) and bool(self._reader) and bool(self._writer):
            templates: Dict[str, str] = self._reader.read(
                self.config, pro_name, verbose
            )
            if bool(templates):
